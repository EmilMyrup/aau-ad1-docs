...
"details" which may be affected by the specific implementatiion and hardware.
...
> Using the basic definition of $\theta$-notation, prove that $max(f(n), g(n)) = \theta(f(n) + g(n))$
...
True. Note that $2^{n+1} = 2 \times 2n$.
...
Solve the exercises in this [file](/images/https://www.moodle.aau.dk/pluginfile.php/373593/mod_resource/content/1/exercises2.pdf).
